...
public class InventoryController : MonoBehaviour {
   ...
   bool CreateAndInsertItem(ItemGrid grid, InventoryItem item) {
      ...
      itemRt = newItem.GetComponent<RectTransform>();
      itemRt.SetParent(canvasTransform);
      itemRt.SetAsLastSibling();

      newItem.Set(item.itemData);
      newItem.name = item.itemData.category.ToString();

      Vector2Int? posOnGrid = grid.FindSpaceForObject(newItem);
      if (posOnGrid == null) {
         return false;
      }

      grid.PlaceItem(newItem, posOnGrid.Value.x, posOnGrid.Value.y);
      return true;
   }
