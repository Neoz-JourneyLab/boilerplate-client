...
namespace Proyecto26.Common {
	public static class Common {
		...
		static string GetFormSectionsContentType(out byte[] bodyRaw, RequestHelper options) {
		...
		static void ConfigureWebRequestWithOptions(UnityWebRequest request, byte[] bodyRaw, string contentType, RequestHelper options) {
#if UNITY_2018_1_OR_NEWER
			if (options.CertificateHandler is CertificateHandler) request.certificateHandler = options.CertificateHandler;
#endif
			if (options.UploadHandler is UploadHandler) request.uploadHandler = options.UploadHandler;
			if (bodyRaw != null) {
			...
			} else request.downloadHandler = new DownloadHandlerBuffer();
			if (!string.IsNullOrEmpty(contentType)) request.SetRequestHeader(CONTENT_TYPE_HEADER, contentType);
			foreach (KeyValuePair<string, string> header in RestClient.DefaultRequestHeaders) {
				request.SetRequestHeader(header.Key, header.Value);
			}
			foreach (KeyValuePair<string, string> header in options.Headers) {
				request.SetRequestHeader(header.Key, header.Value);
			}
			if (options.Timeout.HasValue) request.timeout = options.Timeout.Value;
			if (options.ChunkedTransfer.HasValue) request.chunkedTransfer = options.ChunkedTransfer.Value;
			if (options.UseHttpContinue.HasValue) request.useHttpContinue = options.UseHttpContinue.Value;
			if (options.RedirectLimit.HasValue) request.redirectLimit = options.RedirectLimit.Value;
		...
		public static IEnumerator SendWebRequestWithOptions(this UnityWebRequest request, RequestHelper options) {
			...
			if (!options.Headers.TryGetValue(CONTENT_TYPE_HEADER, out contentType) && options.DefaultContentType) contentType = DEFAULT_CONTENT_TYPE;
			if (options.Body != null || !string.IsNullOrEmpty(options.BodyString)) {
				string bodyString = options.BodyString;
				if (options.Body != null) bodyString = JsonUtility.ToJson(options.Body);
			...
			} else if (options.FormSections != null && options.FormSections.Count > 0) contentType = GetFormSectionsContentType(out bodyRaw, options);
			else if (options.FormData is WWWForm) {
				//The Content-Type header will be copied from the formData parameter
				contentType = string.Empty;
			}
			if (!string.IsNullOrEmpty(options.ContentType)) contentType = options.ContentType;
