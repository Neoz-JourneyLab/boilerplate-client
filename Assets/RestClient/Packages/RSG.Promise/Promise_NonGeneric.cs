...
namespace RSG {
  ...
  public class Promise : IPromise, IPendingPromise, IPromiseInfo {
    ...
    public IPromise Then(Func<IPromise> onResolved, Action<Exception> onRejected, Action<float> onProgress) {
      ...
      Action resolveHandler = () => {
        if (onResolved != null) {
          onResolved().Progress(progress => resultPromise.ReportProgress(progress)).Then(() => resultPromise.Resolve(), ex => resultPromise.Reject(ex));
    ...
    /// </summary>
    public static event EventHandler<ExceptionEventArgs> UnhandledException { add => unhandlerException += value; remove => unhandlerException -= value; }
    ...
    void InvokeProgressHandlers(float progress) {
      if (progressHandlers != null) progressHandlers.Each(handler => InvokeProgressHandler(handler.callback, handler.rejectable, progress));
    }
    ...
    void ActionHandlers(IRejectable resultPromise, Action resolveHandler, Action<Exception> rejectHandler) {
      if (CurState == PromiseState.Resolved) InvokeResolveHandler(resolveHandler, resultPromise);
      else if (CurState == PromiseState.Rejected) InvokeRejectHandler(rejectHandler, resultPromise, rejectionException);
      else {
    ...
    public static IPromise Race(IEnumerable<IPromise> promises) {
      IPromise[] promisesArray = promises.ToArray();
      if (promisesArray.Length == 0) throw new InvalidOperationException("At least 1 input promise must be provided for Race");
