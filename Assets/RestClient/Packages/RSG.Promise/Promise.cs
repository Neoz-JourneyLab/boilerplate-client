	...
	public class Promise<PromisedT> : IPromise<PromisedT>, IPendingPromise<PromisedT>, IPromiseInfo {
		...
		public void Reject(Exception ex) {
			...
			if (CurState != PromiseState.Pending) {
				throw new PromiseStateException("Attempt to reject a promise that is already in state: " +
					CurState +
		...
		public void Resolve(PromisedT value) {
			if (CurState != PromiseState.Pending) {
				throw new PromiseStateException("Attempt to resolve a promise that is already in state: " +
					CurState +
		...
		public void ReportProgress(float progress) {
			if (CurState != PromiseState.Pending) {
				throw new PromiseStateException("Attempt to report progress on a promise that is already in state: " +
					CurState +
					", a promise can only report progress when it is still in state: " +
					PromiseState.Pending);
			}
		...
		Then(onResolved, onRejected).Catch(ex => Promise.PropagateUnhandledException(this, ex));
		...
		public IPromise<ConvertedT> Then<ConvertedT>(Func<PromisedT, IPromise<ConvertedT>> onResolved,
			...
			Action<Exception> rejectHandler = ex => {
				...
				try {
					onRejected(ex).Then(chainedValue => resultPromise.Resolve(chainedValue), callbackEx => resultPromise.Reject(callbackEx));
			...
			ActionHandlers(resultPromise, resolveHandler, rejectHandler);
			if (onProgress != null) ProgressHandlers(this, onProgress);
		...
		public IPromise Then(Func<PromisedT, IPromise> onResolved, Action<Exception> onRejected, Action<float> onProgress) {
			...
			Action<PromisedT> resolveHandler = v => {
				if (onResolved != null) {
					onResolved(v).Progress(progress => resultPromise.ReportProgress(progress)).Then(() => resultPromise.Resolve(), ex => resultPromise.Reject(ex));
		...
		Then(value => Promise<ConvertedT>.Resolved(transform(value)));
		...
		Then(value => Promise<ConvertedT>.All(chain(value)));
		...
		void AddProgressHandler(Action<float> onProgress, IRejectable rejectable) {
			if (progressHandlers == null) progressHandlers = new List<ProgressHandler>();
		...
		void InvokeProgressHandlers(float progress) {
			if (progressHandlers != null) progressHandlers.Each(handler => InvokeHandler(handler.callback, handler.rejectable, progress));
		}
		...
		void ActionHandlers(IRejectable resultPromise, Action<PromisedT> resolveHandler, Action<Exception> rejectHandler) {
			if (CurState == PromiseState.Resolved) InvokeHandler(resolveHandler, resultPromise, resolveValue);
			else if (CurState == PromiseState.Rejected) InvokeHandler(rejectHandler, resultPromise, rejectionException);
			else {
		...
		void ProgressHandlers(IRejectable resultPromise, Action<float> progressHandler) {
			if (CurState == PromiseState.Pending) AddProgressHandler(progressHandler, resultPromise);
		}
		...
		public static IPromise<IEnumerable<PromisedT>> All(IEnumerable<IPromise<PromisedT>> promises) {
			...
			if (promisesArray.Length == 0) return Promise<IEnumerable<PromisedT>>.Resolved(Enumerable.Empty<PromisedT>());
			...
			promisesArray.Each((promise, index) => {
				promise.Progress(v => {
					progress[index] = v;
					if (resultPromise.CurState == PromiseState.Pending) resultPromise.ReportProgress(progress.Average());
				}).
				Then(result => {
					progress[index] = 1f;
		...
		Race((IEnumerable<IPromise<PromisedT>>)promises); // Cast is required to force use of the other function.
